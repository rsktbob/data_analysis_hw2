...
def split_data(data_file, output_dir, mode):
    ...
    if mode == 'holdout':
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
            ...
            dump_svmlight_file(X_train, y_train, os.path.join(output_dir, 'cross_validation', f'train_fold{fold}.scale'))
            dump_svmlight_file(X_val, y_val, os.path.join(output_dir, 'cross_validation', f'val_fold{fold}.scale'))
